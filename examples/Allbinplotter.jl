...
for j in 2:88
    ...
    plot(new_x, Diagonal_element[:, j], seriestype=:stepbins, fill_between=fill(0, size(Diagonal_element, 1)),
        lab="$(wavelist[j,2])", lw=0.25, lc=:black, xticks=0:0.2:10, size(800, 500), legend=:best, legendfontsize=5, m=(1.5, :black, stroke(0.0)))
    # bar!(new_x,err, lab="",fillrange=err[:,2],fillalpha=0.02, c=:orange ,l=nothing)
    bar!(x, qtls[:, 1], lab="Bootstrap Quantiles", fillrange=qtls[:, 2], fillalpha=0.8, c=:orange, l=nothing, xlab="M3pi (GeV)", ylab="Events/20MeV", size(800, 500), title="$(wavelist[j,2])")
