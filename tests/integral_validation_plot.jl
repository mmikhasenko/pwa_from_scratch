...
function CreatePlotInterference(i_misha::Int64, j_misha::Int64, i_florian::Int64, j_florian::Int64, fileextra = "", filefolder = "output", output::Bool = true)
  ...
  plot( mishaij[:,1], mishaij[:,2], label="re(misha)", frame=:origin)
  plot!(mishaij[:,1], mishaij[:,3], label="im(misha)")
  ...
  plot!(florianij[:,1], florianij[:,2], label="re(florian)", title="Interference:  $(florian[i_florian+1,1])...$(florian[j_florian+1,1])")
